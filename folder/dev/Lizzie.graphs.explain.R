...
server <- function(input, output){
  ...
  output$degree.distribution.ferguson <- renderPlot({ # start degree.distribution.ferguson
    ...
    degree.dist.ferguson <- degree_distribution(g.ferguson, cumulative=F, mode="all")
  ...
  output$degree.distribution.orlando <- renderPlot({ # start degree.distribution.orlando
    ...
    degree.dist.orlando <- degree_distribution(g.orlando, cumulative=F, mode="all")
  ...
  #start of degree.distribution.trump
  output$degree.distribution.trump <- renderPlot({ # start degree.distribution.trump
    ...
    degree.dist.trump <- degree_distribution(g.trump, cumulative=F, mode="all")
  ...
  #start of degree.distribution.obama
  output$degree.distribution.obama <- renderPlot({ # start degree.distribution.obama
    dt.obama.sentiment <- dt.reddit.obama[,list(unique(SOURCE_SUBREDDIT))]
    setnames(dt.obama.sentiment, c("V1"), c("SOURCE_SUBREDDIT"))
    total_sentiment_obama <- setDT(dt.reddit.obama)[, .(total_sentiment_obama = sum(Compound_Sentiment)), by = SOURCE_SUBREDDIT]
    ...
    degree.dist.obama <- degree_distribution(g.obama, cumulative=F, mode="all")
    plot.degree.distribution.obama <- 
      plot( x=degree(g.obama), y= degree.dist.obama, pch=20, cex=1.2, col="orange", 
            xlab="Degree", ylab="Frequency")
    plot.degree.distribution.obama
  }) #end degree.distribution.obama
  
  #start of cumexplain 
  output$cumexplain ({renderText(
    ...
    The cumulative node degree distribution shows the cumulative number of nodes of each degree class. 
    The distribution of degree values over all nodes characterizes the network as a whole."
  )
