shinyServer(function(input, output) {
  ...
  output$description_reddit <- renderUI({
    HTML("Reddit is an American social news aggregation, web content rating, and discussion website. Registered members submit content to the site such as links, text posts, and images, which are then voted up or down by other members. Posts are organized by subject into user-created boards called 'subreddits', which cover a variety of topics including news, science, movies, video games, music, books, fitness, food, and image-sharing. Submissions with more up-votes appear towards the top of their subreddit and, if they receive enough votes, ultimately on the site's front page. Despite strict rules prohibiting harassment, Reddit's administrators spend considerable resources on moderating the site. (Source: Wikipedia)"
    )})
  ...
  output$plot.scatter.sentiment.orlando <- renderPlotly ({
    ...
    plot.sentiment.orlando <- plot_ly(data = dt.g.r.sentiment.filter.out.orlando,
                                      x = ~positive_outbound,
                                      y = ~negative_outbound,
                                      marker = list(color = ifelse(dt.g.r.sentiment.filter.out.orlando$name == 
                                                                     input$select.subreddit.orlando, 'rgba(152, 0, 0, .8)', 'rgba(63, 127, 191)'),
  ...
  output$plot.scatter.sentiment.ferguson <- renderPlotly ({
    # Create scatter plot of outbound subreddits
    plot.sentiment.ferguson <- plot_ly(data = dt.g.r.sentiment.filter.out.ferguson,
                                       x = ~positive_outbound,
                                       y = ~negative_outbound,
                                       marker = list(color = ifelse(dt.g.r.sentiment.filter.out.ferguson$name == 
                                                                      input$select.subreddit.ferguson, 'rgba(152, 0, 0, .8)', 'rgba(63, 127, 191)'),
  ...
  output$plot.scatter.sentiment.obama <- renderPlotly ({
    # Create scatter plot of outbound subreddits
    plot.sentiment.obama <- plot_ly(data = dt.g.r.sentiment.filter.out.obama,
                                    x = ~positive_outbound,
                                    y = ~negative_outbound,
                                    marker = list(color = ifelse(dt.g.r.sentiment.filter.out.obama$name == 
                                                                   input$select.subreddit.obama, 'rgba(152, 0, 0, .8)', 'rgba(63, 127, 191)'),
  ...
  output$plot.scatter.sentiment.trump <- renderPlotly ({
    # Create scatter plot of outbound subreddits
    plot.sentiment.trump <- plot_ly(data = dt.g.r.sentiment.filter.out.trump,
                                    x = ~positive_outbound,
                                    y = ~negative_outbound,
                                    marker = list(color = ifelse(dt.g.r.sentiment.filter.out.trump$name == 
                                                                   input$select.subreddit.trump, 'rgba(152, 0, 0, .8)', 'rgba(63, 127, 191)'),
