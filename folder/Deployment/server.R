shinyServer(function(input, output) {
  ...
  output$plot.histogram.orlando <- renderPlotly ({
    plot_ly(x = dt.reddit.orlando[date >= input$dateRange.orlando[1] &
  ...
  output$orlando_distribution <- renderPlotly({
    degree.orlando <- ggplot(f.degree.distribution(dt.reddit.orlando), aes(x = l.ddistribution)) + 
      geom_histogram(binwidth = 1)
    degree.orlando
  }) # end of output of orlando
  output$ferguson_distribution <- renderPlotly({
    degree.ferguson <- ggplot(f.degree.distribution(dt.reddit.ferguson), aes(x = l.ddistribution)) + 
      geom_histogram(binwidth = 1)
    degree.ferguson
  }) # end of output of ferguson
  output$obama_distribution <- renderPlotly({
    degree.obama <- ggplot(f.degree.distribution(dt.reddit.obama), aes(x = l.ddistribution)) + 
      geom_histogram(binwidth = 1)
    degree.obama
  }) # end of output of obama
  output$trump_distribution <- renderPlotly({
    degree.trump <- ggplot(f.degree.distribution(dt.reddit.orlando), aes(x = l.ddistribution)) + 
      geom_histogram(binwidth = 1)
    degree.trump
  }) # end of output of trump
