...
require(ggpubr)
g.june2016 <- induced.subgraph(g.reddit, which(V(g.reddit)$date >= "2016-06-01")) 
g.june2016 <- induced.subgraph(g.june2016, which(V(g.reddit)$date <= "2016-06-20"))
...
V(g.reddit)$color <- c("steel blue", "orange")[V(g.reddit)$type+1]
V(g.reddit)$shape <- c("square", "circle")[V(g.reddit)$type+1]


V(g.reddit)$label <- ""
V(g.reddit)$label[V(g.reddit)$type==F] <- dt.reddit.both$SOURCE_SUBREDDIT[V(g.reddit)$type==F] 

V(g.reddit)$label.cex=.6
V(g.reddit)$label.font=2

test <- plot(g.reddit, vertex.label.color="white", vertex.size=(2-V(g.reddit)$type)*8) 
